...
const exception_t fmt_overflow={"Formatting Overflow"};
...
static void cvt_s(int code, va_list *ap, int put(int c, void *cl), void *cl,
...
static void cvt_d(int code, va_list *ap, int put(int c, void *cl), void *cl,
		unsigned char flags[], int width, int precision){
...
static void cvt_u(int code, va_list *ap, int put(int c, void *cl), void *cl,
		unsigned char flags[], int width, int precision){
...
static void cvt_o(int code, va_list *ap, int put(int c, void *cl), void *cl,
		unsigned char flags[], int width, int precision){
	unsigned m = va_arg(*ap, unsigned);
	char buf[43];
	char *p=buf+sizeof(buf);
...
static void cvt_x(int code, va_list *ap, int put(int c, void *cl), void *cl,
		unsigned char flags[], int width, int precision){
	unsigned m = va_arg(*ap, unsigned);
	char buf[43];
	char *p=buf+sizeof(buf);
...
static void cvt_p(int code, va_list *ap, int put(int c, void *cl), void *cl,
		unsigned char flags[], int width, int precision){
	unsigned long m=(unsigned long)va_arg(*ap, void*);
	char buf[43];
	char *p=buf+sizeof(buf);
	precision = INT_MIN;
	do
...
static void cvt_c(int code, va_list *ap, int put(int c, void *cl), void *cl,
		unsigned char flags[], int width, int precision){
	if(width==INT_MIN)
		width=0;
	if(width<0){
	...
	if(!flags['-'])
		pad(width-1,' ');
	put((unsigned char)va_arg(*ap, int), cl);
	if(flags['-'])
		pad(width-1,' ');
}

static void cvt_f(int code, va_list *ap, int put(int c, void *cl), void *cl,
		unsigned char flags[], int width, int precision){
	...
	sprintf(buf, fmt, va_arg(*ap, double));
...
int fmt_snprint(char *buf, int size, const char *fmt, ...){
	va_list ap;
	int len;
	va_start(ap,fmt);
	len = fmt_vnprint(buf, size, fmt, ap);
	va_end(ap);
	return len;
}
...
void fmt_puts(const char *str, int len, int put(int c, void *cl),
		void *cl, unsigned char flags[], int width, int precision){
	assert(str);
	assert(len>=0);
	assert(flags);
	if(width == INT_MIN)
...
void fmt_putd(const char *str, int len, int put(int c, void *cl),
		void *cl, unsigned char flags[], int width, int precision){
	int sign;
	assert(str);
	assert(len>=0);
